...
const doctorDashboard = async (req, res) => {
    try {
        ...
        appointments.map((item) => {
            // ✅ Only count if completed OR payment done AND not cancelled
        ...
        let patients = []
        appointments.map((item) => {
            if (!item.cancelled && !patients.includes(item.userId)) {
        ...
        const dashData = {
            earnings,
            appointments: appointments.filter(a => !a.cancelled).length, // ← only active
